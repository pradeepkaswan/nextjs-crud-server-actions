...
export const createAccount = async (rawData: FormData) => {
  const data = CreateAccountSchema.parse({ name: rawData.get("name") });
...
const CreateWorkSessionSchema = z.object({
  ...
  description: z.string().min(3).max(120).optional().nullable(),
  hours: z.preprocess(
    nullifyEmptyString,
...
export const deleteWorkSession = async (rawData: FormData) => {
  ...
  const session = await prisma.workSession.delete({
    where: { id: id },
  });
